...
本矩阵以 Yunzai `delta-force-plugin` 的 `apps` 目录为功能范围，以当前三角洲 Go API 文档和源码为接口权威来源，以本地 AstrBot 源码为框架权威来源。2026-08-15 已重新从 GitHub 同步 API 仓库，`delta-force-api-go` 的 `main` 分支最新提交仍为 `fbd0eb1d67731c99b012a999ee96cde47ad976c2`（提交于 2026-07-20）。
...
2026-08-15 对接最新版个人成就与成就分类接口：新增模式、分类与分页语义，展示最高徽章等级、描述和解锁条件；当前注册结构为 108 个独立 `CommandFilter`、369 个唯一命令名与别名。

2026-08-15 对接最新版改枪社区收藏夹与被拒方案复审：新增公开/个人收藏夹查询、详情、CRUD、添加/移除方案和作者复审 9 个独立命令及别名；当时注册结构为 106 个独立 `CommandFilter`、362 个唯一命令名与别名。
...
2026-08-15 对照 Yunzai 敏感消息保护和本地成熟插件实现，为 OAuth 回调增加 aiocqhttp 原生撤回；实际 `AiocqhttpMessageEvent` 测试确认消息 ID 传给 `delete_msg`，其他适配器或撤回失败时返回手动撤回提示，且不影响绑定收尾。

2026-08-15 使用真实 AstrBot v4.17.6 主动消息链路验证：合成 `record.new` 数据经 `_push_record_event`、`Context.send_message()`、UMO 解析和 aiocqhttp 适配器分别路由至群聊与私聊；文本和图片均转换为合法 OneBot 消息段，图片在协议下发前转为 Base64，重复事件未重复发送。最终协议端为内存 Bot，没有向真实会话发送测试消息；真实后端产生的新战绩事件仍待交互复验。
...
2026-08-15 使用 AstrBot v4.17.6 真实依赖完成隔离子进程注册验证：插件类和元数据正常加载，注册器包含 82 个消息 handler、82 个 `CommandFilter`、0 个 `RegexFilter`；308 个命令与别名无重复、无空格名称，每条带参数消息只命中一个 handler。自定义多字符触发符 `!!` 经真实 `WakingCheckStage` 去除后，完整命令参数得到保留且只激活目标 handler。

| 分类 | Yunzai 源文件 | Yunzai fnc | 原命令/别名摘要 | AstrBot 注册命令 | AstrBot 实现函数 | API 方法与路径 | 渲染模板 | 文本兜底 | 成功测试 | 空数据测试 | 错误测试 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 账户 | `apps/account/Account.js` | `showAccounts`、`bindToken`、`unbindToken`、`deleteToken`、`switchAccount`、`refreshWechat`、`refreshQq` | 账号、绑定、解绑、删除、切换、刷新 | 账号、绑定、解绑、账号切换、微信刷新及别名 | `_account_list`、`_bind_token`、`_recall_sensitive_message`、`_delete_account`、`_switch_account`、`_refresh_account` | `GET/POST/DELETE /api/v1/user/bindings` 及主绑定子路由；`GET /api/v1/login/{qq,wechat}/refresh`；`DELETE /api/v1/login/{qq,wechat}/token`；aiocqhttp `delete_msg` | 无 | 有 | 绑定消息撤回、切换、解绑、登录删除和刷新 fixture 通过 | 空账号列表 fixture 通过 | 非 aiocqhttp 提示、响应脱敏、远端失败不改本地及类型不匹配 fixture 通过 | 已验证 |
| 登录 | `apps/account/Login.js` | `login`、`loginWithCookie`、`qqOAuthLogin`、`wechatOAuthLogin`、`webLogin`、`bindCharacter` | 扫码登录、CK 登录、QQ/微信 OAuth、网页登录、角色绑定、敏感回调撤回 | 登录、ck登录、qq授权登录、微信授权登录、网页登录、角色绑定；完整兼容各平台“登陆/登录”与 OAuth 字面组合 | `_login`、`_cookie_login`、`_oauth_login`、`_recall_sensitive_message`、`_recall_oauth_callback`、`_web_login`、`_bind_character` | `GET /api/v1/login/{platform}/qr`；`GET /api/v1/login/{platform}/status`；`POST /api/v1/login/qq/ck`；`GET/POST /api/v1/login/{qq,wechat}/oauth`；`POST/GET /api/v1/authorization/requests` 及状态子路由；`GET /api/v1/df/person/bind?method=bind`；aiocqhttp `delete_msg` | 无 | 有 | 二维码组件、动态有效期、Cookie/OAuth/显式角色绑定敏感消息撤回、网页登录领取、账号与角色自动绑定及别名集合 fixture 通过 | 缺少二维码与未绑定账号 fixture 通过 | 过期二维码、授权拒绝/过期/重复领取、API 错误、state 不匹配、撤回失败、非 aiocqhttp 提示、登录收尾与响应脱敏、远端绑定失败和凭证失效 fixture 通过 | 部分（仍需真实扫码与网页授权复验） |
...
| 报告 | `apps/report/RecordSubscription.js` | `subscribeRecord`、`unsubscribeRecord`、`getSubscriptionStatus`、`enableGroupPush`、`disableGroupPush`、`enablePrivatePush`、`disablePrivatePush` | 战绩订阅、状态、群/私聊推送开关 | 合法根命令 `订阅`、`取消订阅`、`订阅状态`，以 `战绩` 为参数 | `_record_subscription`、`_subscription_target`、`_ws_supervisor`、`_push_record_event` | `GET/POST /api/v1/user/record-subscriptions`；详情、删除、启停、事件、最近战绩子路由；WS `/ws` 的 `record.client.subscribe`、`record.new` | `recordPush` | 有 | 创建、取消、状态、类型切换、SOL/MP 图片推送、事件去重、多目标及重载任务去重 fixture 通过；真实客户端级订阅确认和 AstrBot 主动消息路由通过 | 无订阅、无订阅 ID、无后端绑定 fixture 通过 | REST、删除旧订阅、目标查询、订阅拒绝、图片渲染失败、多目标发送和资源关闭异常隔离 fixture 通过 | 部分（真实握手、频道订阅及框架下发链路已验证，仍需实际 `record.new` 事件复验） |
...
| 系统 | `apps/system/Help.js` | `help`、`entertainmentHelp` | 帮助/菜单/功能、娱乐帮助/菜单/功能 | 帮助、娱乐帮助、计算帮助及完整别名 | `_help`、`_calculator_help` | 无 | `help/index.html` | 有 | 字面别名、Playwright fixture 通过 | 不适用 | 配置读取失败 fixture 通过 | 已验证 |
| 系统 | `apps/system/Update.js` | `update`、`update_log` | 更新、插件更新、强制更新、插件强制更新、更新日志、插件更新日志 | 更新及完整别名 | `_update_plugin`、`_update_log`、`_parse_changelog` | AstrBot 原生 `StarManager.update_plugin("sanjiaozhou")`；本地 `CHANGELOG.md` | `help/version-info.html` | 有 | 完整别名、原生更新管理器、最近两版解析、图片组件与 Playwright fixture 通过 | 空日志与管理器缺失 fixture 通过 | 非管理员、更新失败、日志读取和渲染失败 fixture 通过 | 已验证 |
| 系统 | `apps/system/WebSocketClient.js` | `connectWebSocket`、`disconnectWebSocket`、`getWebSocketStatus` | WebSocket 连接、断开、状态 | ws连接及大量别名 | `_ws_supervisor`、`_ws_run_once`、`_push_record_event` | `/ws`；`X-API-Key` 握手；`record.client.subscribe`；`record.new` | 无 | 有 | 权威协议、零值 `code` 省略、订阅确认、事件分发、去重、多目标与重载任务替换 fixture 通过；真实握手、频道确认及 AstrBot 主动消息路由通过 | 不适用 | `kind=error` 拒绝、非 JSON、消费异常、关闭超时与发送失败隔离 fixture 通过 | 部分（连接、订阅及框架下发链路已验证，仍需实际 `record.new` 事件复验） |
...
| API 扩展 | 无（最新版个人数据） | 无 | 无 Yunzai 旧命令 | 成就、荣誉、徽章、成就分类及别名 | `_honors`、`_honor_boxes` | `GET /api/v1/df/person/honor`；`GET /api/v1/df/person/honorbox`；Query：`solutionType=sol/mp`；Header：`X-Framework-Token` | 无 | 有 | 模式、分类名称/ID、分页、最高徽章等级和独立命令分发 fixture 通过 | 空成就与空分类 fixture 通过 | 凭证失效、分类缺失、页码越界和分类元数据降级 fixture 通过 | 已验证 |
...
- 真实 AstrBot v4.17.6 源码导入成功；当前注册结构检查为 108 个独立命令 handler、369 个命令名/别名。
- 注册结果中空格命令、重复命令名和 `RegexFilter` 数量均为 0。
- 脱敏登录二维码 fixture 为 `328×328` PNG，可由图片解析器识别并通过 `Image.fromBase64()` 路径发送。
- 帮助菜单已由插件自身的 `DeltaRenderer` 和 Playwright 渲染为 `2560×8628` PNG，已人工检查新增成就命令、中文、背景、图标、换行和底部完整性；最近两版更新卡片为 `1440×2750` PNG，同样无重叠或裁切。
