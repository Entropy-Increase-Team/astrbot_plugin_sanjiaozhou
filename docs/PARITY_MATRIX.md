...
| 系统 | `apps/system/WebSocketClient.js` | `connectWebSocket`、`disconnectWebSocket`、`getWebSocketStatus` | WebSocket 连接、断开、状态 | ws连接及大量别名 | `_ws_supervisor`、`_ws_run_once`、`_push_record_event` | `/ws`；`record.client.subscribe`；`record.new` | 无 | 有 | 协议信封、事件分发、去重与多目标 fixture 通过 | 不适用 | 非 JSON、消费异常清理与发送失败隔离 fixture 通过 | 部分（仍需部署环境真实连接复验） |
