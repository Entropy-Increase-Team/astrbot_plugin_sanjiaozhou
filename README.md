...
- 当前版本：`0.4.20`，详细变更见 [更新日志](CHANGELOG.md)。
...
- 改枪码社区写操作要求 API Key 具备 `gunmod:community:write` 权限；上传命令必须提供武器 ID 和配件 JSON，方案 ID 使用后端返回的 UUID，评论最多 500 字且只能编辑或删除自己的评论。
...
发送 `帮助` 查看基础菜单，发送 `娱乐帮助` 查看娱乐菜单，发送 `计算帮助` 查看计算器菜单。`0.4.19` 将藏品查询升级为聚合个人资产接口；`0.4.20` 补齐最新版改枪社区的我的方案、评论、浏览和复制能力。
...
- [x] 利润历史和利润排行
- [x] 特勤处利润
- [x] 改枪码社区列表、详情、上传、更新、删除、投票、收藏、我的方案、评论、浏览和复制
- [x] 伤害计算
- [x] 维修/修甲计算
- [x] 计算映射表
