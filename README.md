...
- 当前版本：`0.4.48`，详细变更见 [更新日志](CHANGELOG.md)。
...
发送 `帮助` 查看基础菜单，发送 `娱乐帮助` 查看娱乐菜单，发送 `计算帮助` 查看计算器菜单。`0.4.48` 已对齐多物品价格语义：当前价格最多 5 个、价格历史最多 3 个、材料价格最多 5 个、利润历史最多 10 个，支持名称、ID、半角/全角逗号和 JSON 数组混合输入。
