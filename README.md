...
- 当前版本：`0.4.45`，详细变更见 [更新日志](CHANGELOG.md)。
...
发送 `帮助` 查看基础菜单，发送 `娱乐帮助` 查看娱乐菜单，发送 `计算帮助` 查看计算器菜单。`0.4.45` 已对齐鼠鼠音乐的 2 分钟记忆、每页 10 首和跨页全局序号，列表第二页可直接使用 `点歌 11` 等当前页序号播放。
...
| `战备` | 启动交互式最低成本配装计算 | `战备` |
| `语音 [角色/标签/分类] [场景] [动作]` | 按角色、标签、分类、场景和动作播放语音 | `语音 红狼 局内 战斗` |
